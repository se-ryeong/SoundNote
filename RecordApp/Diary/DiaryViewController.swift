...
final class DiaryViewController : UIViewController {
    ...
    func setNavigationBar() {
        let backBarButtonItem = UIBarButtonItem(title: "Back".localized, style: .plain, target: self, action: nil)
    ...
    func recordingFinishAlert() {
        singleActionAlert(title: "The recording is complete.".localized,
                          message: "") { [weak self] in
            guard let self else { return }
            shouldSave = true
            memoView.textView.isEditable = true
            recordButton.setTitle("Save".localized, for: .normal)
        }
    }
    
    func finishRecoding() {
        singleActionAlert(title: "Memo has been saved.".localized,
